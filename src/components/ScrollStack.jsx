...
const ScrollStack = ({
  ...
  // Recalculate static positions of all cards relative to document or scroller
  const measureStaticPositions = useCallback(() => {
    if (!scrollerRef.current) return;
    const cards = cardsRef.current;
    if (!cards.length) return;

    if (useWindowScroll) {
      // Temporarily read static layout top
      const scrollerRect = scrollerRef.current.getBoundingClientRect();
      const scrollerTop = scrollerRect.top + window.scrollY;
      
      initialTopsRef.current = cards.map(card => {
        return scrollerTop + card.offsetTop;
      });

      const endElement = scrollerRef.current.querySelector('.scroll-stack-end');
      if (endElement) {
        endElementTopRef.current = scrollerTop + endElement.offsetTop;
      }
    } else {
      initialTopsRef.current = cards.map(card => card.offsetTop);
      const endElement = scrollerRef.current.querySelector('.scroll-stack-end');
      if (endElement) {
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const pinEnd = endElementTop > 0 ? (endElementTop - containerHeight / 2) : pinStart + 800;
      ...
      if (blurAmount) {
        ...
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = initialTopsRef.current[j] ?? cardsRef.current[j].offsetTop;
  ...
  useLayoutEffect(() => {
    ...
    // Also listen to native window scroll as fallback / synchronization
